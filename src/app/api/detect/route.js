...
const base_model_url = process.env.NEXT_PUBLIC_BASE_MODEL_URL;
...
export async function POST(request) {
  ...
  try {
    ...
    const isValidApiKey = await ValidateApiKey(apiKey, 'userdata');
...
async function detectDiseaseApi(formdata) {
  ...
  const response = await fetch(`${base_model_url}/detect`, {
    method: 'POST',
    headers: {
